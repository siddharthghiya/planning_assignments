...
bool goalReached(node* currentPtr, double* target_traj, int target_steps){
    // check from current time step(curr_time) to the end time step(target_steps), if any of the goal state has been expanded
    int time;
    time = currentPtr->t;
    if ((currentPtr->posX == target_traj[time]) && (currentPtr->posY == target_traj[time+target_steps])){
            return true;
        }
    return false;
}
...
{   
    ...
    while(!open_list.empty()){
        ...
        if (goalReached(currentPtr, target_traj, target_steps)){
            break;
        }
